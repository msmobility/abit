...
OUTPUT_DIR = r"C:\Users\Nawidullah\IdeaProjects\abit\viz\work_destination_maps"
os.makedirs(OUTPUT_DIR, exist_ok=True)

OUTPUT_BASE_PNG = os.path.join(OUTPUT_DIR, "base_home_to_work_trip_destinations_by_zone.png")
OUTPUT_COMPARE20_PNG = os.path.join(OUTPUT_DIR, "telework20_minus_base_home_to_work_trip_destinations_by_zone.png")
OUTPUT_COMPARE40_PNG = os.path.join(OUTPUT_DIR, "telework40_minus_base_home_to_work_trip_destinations_by_zone.png")
OUTPUT_COMPARE80_PNG = os.path.join(OUTPUT_DIR, "telework80_minus_base_home_to_work_trip_destinations_by_zone.png")
...
# STYLE
# =========================================================
# Scientific / publication-style colormaps
BASE_CMAP = "Blues"
...
def compute_home_work_trip_counts(legs_csv, dest_zone_col, scenario_name):
    ...
    # keep only valid destination zone + valid coords
    legs = legs[
...
def plot_base_map(zones_gdf, output_png):
    ...
    zones_gdf.plot(
        ...
        legend_kwds={"label": "Number of HOME→WORK trips ending in zone"}
    )

    ax.set_title("Base Scenario: HOME→WORK Trip Destinations by Zone", fontsize=TITLE_SIZE)
    ax.set_axis_off()
...
zones[ZONE_ID_COL_SHP] = zones[ZONE_ID_COL_SHP].astype(str).str.strip()
...
if PLOT_BASE and (PLOT_TELE20 or PLOT_TELE40 or PLOT_TELE80):
    ...
    change["change_20_abs"] = change["tele20_count"] - change["base_count"]
    change["change_40_abs"] = change["tele40_count"] - change["base_count"]
    change["change_80_abs"] = change["tele80_count"] - change["base_count"]
    ...
    for col in [
        "base_count", "tele20_count", "tele40_count", "tele80_count",
        "change_20_abs", "change_40_abs", "change_80_abs"
    ]:
    ...
    max_abs_change = max(
        zones_compare["change_20_abs"].abs().max() if PLOT_TELE20 else 0,
        zones_compare["change_40_abs"].abs().max() if PLOT_TELE40 else 0,
        zones_compare["change_80_abs"].abs().max() if PLOT_TELE80 else 0
    )
    ...
    if PLOT_TELE20:
        plot_change_map(
            ...
            change_col="change_20_abs",
            title="20% Telework vs Base: Change in HOME→WORK Trip Destinations by Zone",
            legend_label="Absolute change in HOME→WORK trips ending in zone (20% telework - base)",
    ...
    if PLOT_TELE40:
        plot_change_map(
            zones_gdf=zones_compare,
            change_col="change_40_abs",
            title="40% Telework vs Base: Change in HOME→WORK Trip Destinations by Zone",
            legend_label="Absolute change in HOME→WORK trips ending in zone (40% telework - base)",
    ...
    if PLOT_TELE80:
        plot_change_map(
            zones_gdf=zones_compare,
            change_col="change_80_abs",
            title="80% Telework vs Base: Change in HOME→WORK Trip Destinations by Zone",
            legend_label="Absolute change in HOME→WORK trips ending in zone (80% telework - base)",
