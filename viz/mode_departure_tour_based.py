...
def prepare_mode_departure_counts(tours_csv, scenario_name, allowed_modes, label_name):
    print(f"\nLoading scenario: {scenario_name} | mode group: {label_name}")
    ...
    required_cols = ["tour_start_time_min", "day", "tour_mode"]
    ...
    df["tour_mode"] = df["tour_mode"].astype(str).str.upper().str.strip()

    df = df.dropna(subset=["tour_start_time_min", "day"]).copy()

    print("Rows after cleaning:", len(df))

    # Keep only selected tour modes
    mode_df = df[df["tour_mode"].isin(allowed_modes)].copy()

    print(f"{label_name.upper()} tours kept:", len(mode_df))
    print("Modes found after filter:", sorted(mode_df["tour_mode"].dropna().unique().tolist()))

    mode_df["day_index"] = mode_df["day"].astype(int)

    # Keep only days 1..7
    mode_df = mode_df[
        (mode_df["day_index"] >= 1) & (mode_df["day_index"] <= 7)
        ].copy()

    print("Rows after restricting to days 1..7:", len(mode_df))
    print("Derived day indices:", sorted(mode_df["day_index"].unique()))

    mode_df["minute_in_day"] = mode_df["tour_start_time_min"] % 1440
    mode_df["time_bin"] = (
            (mode_df["minute_in_day"] // BIN_MINUTES) * BIN_MINUTES
    ).astype(int)

    mode_df["scenario"] = scenario_name

    counts = (
        mode_df.groupby(["scenario", "day_index", "time_bin"])
        .size()
...
def add_percent_vs_baseline(day_df, baseline_day_total):
    ...
    if baseline_day_total > 0:
        day_df["pct_vs_base"] = 100.0 * day_df["n_tours"] / baseline_day_total
    else:
...
def load_all_scenarios_for_mode(mode_key, allowed_modes):
    base_counts = prepare_mode_departure_counts(BASE_TOURS_CSV, SCENARIO_BASE, allowed_modes, mode_key) if PLOT_BASE else None
    tele20_counts = prepare_mode_departure_counts(TELE20_TOURS_CSV, SCENARIO_TELE20, allowed_modes, mode_key) if PLOT_TELE20 else None
    tele40_counts = prepare_mode_departure_counts(TELE40_TOURS_CSV, SCENARIO_TELE40, allowed_modes, mode_key) if PLOT_TELE40 else None
    tele80_counts = prepare_mode_departure_counts(TELE80_TOURS_CSV, SCENARIO_TELE80, allowed_modes, mode_key) if PLOT_TELE80 else None
...
def plot_mode_group(mode_key, allowed_modes):
    ...
    for d in range(1, 8):
        ...
        if PLOT_BASE:
            ...
            baseline_day_total = int(base_day["n_tours"].sum())
            ...
            ax.fill_between(
                ...
                alpha=0.95,
                label=f"{SCENARIO_BASE} area (daily {mode_key.upper()} tours n={baseline_day_total})"
            )
            ...
            print(f"{SCENARIO_BASE}: daily {mode_key.upper()} tours total={baseline_day_total}")

        if PLOT_TELE20:
            tele20_day = tele20_counts[tele20_counts["day_index"] == d].sort_values("time_bin").copy()
            total_tele20 = int(tele20_day["n_tours"].sum())
            ...
            ax.plot(
                ...
                label=f"{SCENARIO_TELE20} (daily {mode_key.upper()} tours n={total_tele20}, total vs base={pct_change_20:+.1f}%)"
            )

            print(f"{SCENARIO_TELE20}: daily {mode_key.upper()} tours total={total_tele20}")

        if PLOT_TELE40:
            tele40_day = tele40_counts[tele40_counts["day_index"] == d].sort_values("time_bin").copy()
            total_tele40 = int(tele40_day["n_tours"].sum())
            ...
            ax.plot(
                ...
                label=f"{SCENARIO_TELE40} (daily {mode_key.upper()} tours n={total_tele40}, total vs base={pct_change_40:+.1f}%)"
            )

            print(f"{SCENARIO_TELE40}: daily {mode_key.upper()} tours total={total_tele40}")

        if PLOT_TELE80:
            tele80_day = tele80_counts[tele80_counts["day_index"] == d].sort_values("time_bin").copy()
            total_tele80 = int(tele80_day["n_tours"].sum())
            ...
            ax.plot(
                ...
                label=f"{SCENARIO_TELE80} (daily {mode_key.upper()} tours n={total_tele80}, total vs base={pct_change_80:+.1f}%)"
            )

            print(f"{SCENARIO_TELE80}: daily {mode_key.upper()} tours total={total_tele80}")

        ax.set_title(f"Daily {mode_key.upper()} departure times relative to baseline ({BIN_MINUTES}-min bins) - {day_labels[d]}")
        ax.set_xlabel("Departure time")
        ax.set_ylabel(f"{mode_key.upper()} departures as share of baseline day total (%)")
        ...
        output_file = os.path.join(
            OUTPUT_DIR,
            f"{mode_key}_departure_share_vs_base_{BIN_MINUTES}min_{day_labels[d]}.png"
        )
